...
def print_average(size, tbt, ttft):
    ...
    ratio_list = ["30-70-0-100-100-0"]
    cache_device = ["gpu"]

    # Initialize a dictionary to store the results
    results = {}

    # Itcd erate over all batch sizes and sequence lengths
    for b in batch_sizes:
        for seq_len in sequence_lengths:
            for ratio in ratio_list:
                for device in cache_device:
                    # Construct the filename
                    #file_name = f"./{name}_{size}b_xft_b{b}_i{seq_len}"
                    file_name = f"/scale/cal/home/gwanhoga/FlexGen/flexgen/logfile_0906/fo-{size}b-gbs{b}-ngbs1-prompt{seq_len}-gen32-percent-{ratio}-{device}-cache.log"
                    ...
                    if os.path.exists(file_name):
                        with open(file_name, 'r') as log_file:
                            ...
                            results[(b, seq_len, ratio, device)] = {'TTFT_avg': prefill_latency, 'TBT_avg': decode_latency}
                    else:
    ...
    if tbt == True:
        ...
        for(b, seq_len, ratio, device), averages in results.items():
            #print(f"{b:<10} {seq_len:<10} {averages['TTFT_avg']:<15.2f} {averages['TBT_avg']:<15.2f}")
            print(f"model size:{size}, batch size:{b}, prompt length:{seq_len}, ratio:{ratio}, KVcache device:{device}")
...
def main():
    ...
    parser.add_argument('-b', '--msize', type=float, help='model size')
